...
void *Communication_server::accept_connections()
{
    ...
    this->username = this->username.substr(0, username.length() - 1);

    // Create client folder, if it doesn't already exist
    create_folder("/home/" + username + "/sync_dir");
...
void *Communication_server::receive_commands(int sockfd)
{
    ...
    string path = "/home/" + username + "/sync_dir";
    ...
    {
        cout << "\ncommand 1 received\n";
        path = path + receive_payload(sockfd)->_payload;
    ...
    case 3: //Delete server
    {
       /* path = path + receive_payload(sockfd)->_payload;
        if (unlink(path.c_str()) == -1)
                    std::cout << "Error on Delete file: " << path << "\n";
